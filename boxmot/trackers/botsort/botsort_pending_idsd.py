...
class BotSort(BaseTracker):
    ...
    def __init__(
        ...
        # NOTE: Do NOT clear global BaseTrack ID counter in multi-camera mode
        # to avoid cross-tracker ID resets/collisions.
    ...
    def _idsd_single_fast_from_cluster(
        ...
        if lost_pool:
            ...
            if jbest >= 0 and float(col[jbest]) <= float(self.single_lost_match_thresh):
                ...
                if self.with_reid:
                    feat_bank = getattr(lost_trk, "curr_feat", None) or self._feat_of(t)
