...
class BotSort(BaseTracker):
    ...
    def _first_association(
        ...
        if self.with_reid:
            ...
            dists = np.where(
                np.isnan(ious_dists) & ~np.isnan(emb_dists), emb_dists,
                np.where(
                    ~np.isnan(ious_dists) & np.isnan(emb_dists), ious_dists,
                    np.fmin(ious_dists, emb_dists)  # dùng fmin để tránh lan NaN
                )
            )
    ...
    def _prepare_output(
        ...
        return np.asarray(outputs), logs