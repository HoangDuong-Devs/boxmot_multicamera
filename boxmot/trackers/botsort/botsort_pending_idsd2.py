...
class BotSort(BaseTracker):
    ...
    def __init__(
        ...
        # NOTE: Do NOT clear global BaseTrack ID counter here. In multi-camera/multi-thread
        # setups this would reset IDs across trackers and can lead to collisions.
        ...
        # Each tracker keeps its own Kalman filter to avoid sharing state across trackers.
        ...
        env_uid = os.environ.get("BOXMOT_RUN_UID")
        self.run_uid = env_uid if env_uid else _uuid.uuid4().hex[:8]
        try:
        ...
        # IDs bị đánh dấu tạm thời trong frame khi xử lý cluster        
    ...
    def _assign_local_id(self, track: STrack) -> int:
        """Issue a tracker-local ID for a newly created track."""
        self._local_id_counter += 1
        track._forced_id = int(self._local_id_counter)
        return track._forced_id
    ...
    def _bank_add_feature(self, track: STrack, feat: np.ndarray):
        ...
        try:
            ...
            extra = {
                ...
                "cls": int(getattr(track, "cls", 0)),
                "camera_id": self.camera_id,
            }
            self.long_bank.add_ring(
                self.run_uid,
                int(track.id),
                v,
                slot,
    ...
    def _bank_centroid_for_track(self, track: STrack):
        ...
        # fetch from bank
        try:
            V, _ = self.long_bank.get_vectors_with_payload(
                self.run_uid, tid, camera_id=self.camera_id
            )
            if V is not None and len(V) > 0:
                c = V.mean(axis=0)
    ...
    def allow_update(self, long_val, track, other_tracks=None):
        """Check if track updates are allowed based on various conditions."""
    ...
    def _idsd_single_fast_from_cluster(
        self,
        t: STrack, i: int,
        F: np.ndarray, fvalid: np.ndarray,
        C_self: np.ndarray,
        lost_pool, C_lost: np.ndarray,
        activated_stracks, lost_stracks,
        img=None
    ):
        # 1) self-recovery từ C[i, i] (không tính lại)
        recovery_th = self.long_update_cos_thresh * 1.2
        self_cost = float(C_self[i, i]) if np.isfinite(C_self[i, i]) else 1.0
        if fvalid[i] and self_cost <= recovery_th:
            # reset cờ nghi ngờ (y hệt _handle_single_suspect_recovery)
            t._suspect_run = 0
            t._in_holder_window = False
            t._holder_win_left = 0
            t._snapshot = None
            t._snap_taken = False
            if hasattr(t, "_holder_votes"):
                t._holder_votes.clear()
            return "recovered"
        
        # 2) long-only reclaim từ cột i của C_lost (đã batch)
        if lost_pool:
            col = C_lost[:, i].copy()
            # coexistence gate
            curr_id = int(getattr(t, "id", -1))
            if curr_id >= 0:
                block = self.coex_map.get(curr_id, set())
                for j, lt in enumerate(lost_pool):
                    if int(getattr(lt, "id", -1)) in block:
                        col[j] = 1.0
                        
            jbest = int(np.argmin(col)) if col.size else -1
            if jbest >= 0 and float(col[jbest]) <= float(self.single_lost_match_thresh):
                # re-activate lost id, và mark t lost theo IDSD
                lost_trk = lost_pool[jbest]
                lost_trk.re_activate(t, self.frame_count, new_id=False, img=img)
                lost_trk.quarantined_until = 0
                if t in getattr(self, "active_tracks", []):
                    self.active_tracks.remove(t)
                self._idsd_mark_lost(t, lost_stracks, reason="single_reclaim_fast")

                # (tuỳ chọn) push 1 vector an toàn vào bank
                if self.with_reid:
                    feat_bank = first_not_none(
                        getattr(lost_trk, "curr_feat", None),
                        getattr(t,        "curr_feat", None),
                        self._feat_of(t)
                    )
                    ...
                    if feat_bank is not None:
                        ...
                        safe = True
                        if ok and proto_vec is not None:
                            safe = (self._cos_cost_simple(feat_bank, proto_vec) 
                                    <= float(self.long_update_cos_thresh))
                        if safe and int(lost_trk.id) not in getattr(self, "_idswap_locked_ids", set()):
                            self._bank_add_feature(lost_trk, feat_bank)
                return "reclaimed"

        # 3) không match → LOST + tạo ID mới (dùng luôn F[i])
        self._idsd_mark_lost(t, lost_stracks, reason="single_newid_fast")
        f = None
        if fvalid[i] and F is not None:
            f = F[i].astype(np.float32, copy=False)
    ...
    def _apply_idswap_cluster(self, cluster_tracks, lost_stracks, activated_stracks, refind_stracks, img=None):
        """
        ...
        # LOST pool & long-cost batch: (N_lost, m)
        lost_pool_now = joint_stracks(self.lost_stracks, lost_stracks)
        if lost_pool_now:
            C_lost = self._compute_long_cost(lost_pool_now, cluster_tracks)
            C_lost = np.nan_to_num(C_lost, nan=1.0, posinf=1.0, neginf=1.0)
        else:
            C_lost = np.empty((0, m), dtype=np.float32)
        
        # Gán trong cụm để biết winner/loser trước
        ...
        # --- Pha 1: xử lý LOSER trước bằng fast-single flow (reuse C, F, C_lost)
        for r in range(m):
            # loser: không được gán hoặc cost > ath
            if new_ids[r] is None or assigned_cost[r] > ath:
                self._idsd_single_fast_from_cluster(
                    t=cluster_tracks[r], i=r,
                    F=F, fvalid=fvalid,
                    C_self=C,
                    lost_pool=lost_pool_now, C_lost=C_lost,
                    activated_stracks=activated_stracks, lost_stracks=lost_stracks,
                    img=img
                )
    ...
    def update(
        ...
        _, _, u_detection_second = self._second_association(
        ...
        if clusters:
            ...
            for cl in clusters:
                self._apply_idswap_cluster(cl, lost_stracks, activated_stracks, refind_stracks, img)
            ...
            for p_track in promotables:
                try:
                    ...
                    if len(cand_ids) == 0:
                        ...
                        self._assign_local_id(new_track)
    ...
    def _prepare_output(
        ...
        if self.long_bank is not None:
            for t in removed_stracks:
                try:
                    if getattr(t, "id", None) is not None:
                        self.long_bank.delete_track(
                            self.run_uid, int(t.id), camera_id=self.camera_id
                        )
