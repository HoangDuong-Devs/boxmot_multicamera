...
class BotSort(BaseTracker):
    ...
    def __init__(
        ...
        self.long_update_cos_thresh   = 0.13 # Feature drift    -> không cập nhật
        self.max_obs = 50
        ...
        # NEW: TTL giữ cụm khi holder biến mất tạm thời
        self.holder_grace_ttl   = 7
    ...
    def _try_match_single_to_lost_long_only(self, single_track: STrack, lost_pool, img=None) -> bool:
        try:
            ...
            # Long-term cost (1 single cột)
            C = self._compute_long_cost(lost_pool, [single_track]).ravel()
            C = np.nan_to_num(C, nan=1.0, posinf=1.0, neginf=1.0)

            # DEBUG: report top-1, min, argmin for single->lost matching
            if getattr(self, 'debug_single', False):
                try:
                    import math
                    best_idx = int(np.argmin(C)) if C.size else -1
                    best_cost = float(C[best_idx]) if (best_idx >= 0 and C.size) else math.nan
                    costs_str = ",".join([f"{x:.3f}" for x in C]) if C.size else "[]"
    ...
    def _update_suspect_holder_state(self, tracks, C, pvalid, fvalid):
        ...
        GTTL    = int(getattr(self, "holder_grace_ttl", 7))  # NEW

        # NEW: map id->index hiện có (để rửa phiếu + tránh StopIteration)
        id2idx = {
            int(t.id): j
            for j, t in enumerate(tracks)
            if getattr(t, "id", None) is not None
        }
        live_ids = set(id2idx.keys())

        # NEW: dựng hàng xóm “sống” theo ngưỡng hold cho từng i
        neighbors_live = []
        for i, ti in enumerate(tracks):
            neigh = set()
            if pvalid[i] and fvalid[i]:
                for j, tj in enumerate(tracks):
                    if i == j: 
                        continue
                    if pvalid[j] and fvalid[j] and C[i, j] <= th_hold:
                        # chỉ tính hàng xóm hiện đang sống
                        tid = getattr(tj, "id", None)
                        if tid is not None and int(tid) in live_ids:
                            neigh.add(int(tid))
            neighbors_live.append(neigh)
        ...
        for i, t in enumerate(tracks):
            ...
            # NEW: bộ nhớ hàng xóm & TTL giữ cụm
            if not hasattr(t, "_last_live_neighbors"):  t._last_live_neighbors = set()
            if not hasattr(t, "_holder_grace_left"):    t._holder_grace_left   = 0
            ...
            if is_occ and t._occlusion_frames < max_occ:
                # vẫn pause, nhưng vẫn cập nhật grace để cụm không vỡ
                # (không xử lý vote trong pause)
                # NEW: cập nhật grace theo biến mất hàng xóm
                prev_nei = set(getattr(t, "_last_live_neighbors", set()))
                now_nei  = set(neighbors_live[i])
                if len(now_nei) == 0:
                    if len(prev_nei) > 0 and t._holder_grace_left == 0:
                        t._holder_grace_left = GTTL
                    elif t._holder_grace_left > 0:
                        t._holder_grace_left -= 1
                else:
                    t._holder_grace_left = 0
                t._last_live_neighbors = now_nei
                continue

            # --- RỬA PHIẾU mồ côi theo live_ids (tránh StopIteration) ---
            if t._holder_votes:
                t._holder_votes = {
                    int(hid): vs
                    for hid, vs in t._holder_votes.items()
                    if isinstance(hid, (int, np.integer)) and hid in live_ids
                }
            ...
            if not (pvalid[i] and fvalid[i]):
                ...
                t._grace_period = 0
                # NEW: update grace snapshot (vẫn theo dõi cụm)
                prev_nei = set(getattr(t, "_last_live_neighbors", set()))
                now_nei  = set(neighbors_live[i])
                if len(now_nei) == 0:
                    if len(prev_nei) > 0 and t._holder_grace_left == 0:
            ...
            if self_cost > th_self:
                ...
                t._holder_win_left = 0
                t._grace_period = 0
                # NEW: tự hồi thì cũng dừng grace
                t._holder_grace_left = 0
                t._last_live_neighbors = set(neighbors_live[i])
                continue
            ...
            if t._suspect_run >= consec:
                ...
                if not t._in_holder_window:
                    ...
                    t._holder_win_left -= 1

                # vote các holder ứng viên (chỉ theo neighbors_live đã lọc)
                for j, other in enumerate(tracks):
                    ...
                    if pvalid[j] and fvalid[j] and C[i, j] <= th_hold:
                        ...
                        if oid is None:
                            continue
                        oid = int(oid)
                        if oid in live_ids:  # chỉ vote holder đang sống
                            t._holder_votes.setdefault(oid, []).append(1)
                ...
                # NEW: cập nhật grace theo hàng xóm
                prev_nei = set(getattr(t, "_last_live_neighbors", set()))
                now_nei  = set(neighbors_live[i])
                if len(now_nei) == 0:
                    if len(prev_nei) > 0 and t._holder_grace_left == 0:
                ...
                # CHANGED: chốt cặp với guard (không dùng next(...) trần)
                if best_hid is not None and best_score >= need:
                    j_idx = id2idx.get(int(best_hid))
                    if j_idx is not None:
                        pairs.append((i, j_idx))
                    else:
                        # holder vừa biến mất → xoá phiếu stale để không crash lần sau
                        t._holder_votes.pop(int(best_hid), None)
                elif t._holder_win_left == 0:
                    # CHANGED: chỉ coi là single khi hết TTL giữ cụm
                    if int(getattr(t, "_holder_grace_left", 0)) == 0:
                        singles_idx.append(i)
                    # nếu còn grace → không thêm vào singles, chờ frame sau
                    # (không reset votes, vẫn giữ snapshot)
    ...
    def update(
        ...
        # --- 4. Suspect–Holder voting & cluster (IDSD Logic) ---
        ...
        if clusters:
            ...
            for cl in clusters:
                self._apply_idswap_cluster(cl, new_tracks_buf, lost_stracks, activated_stracks, refind_stracks, img=img)

            # vệ sinh buffers theo id các track vừa bị đấy sang lost
            if lost_stracks:
                lost_ids = {int(getattr(t, "id", -1)) for t in lost_stracks if getattr(t, "id", None) is not None}
                if lost_ids:
                    self._purge_from_coex(lost_ids)
                    
            for det_like, feat in new_tracks_buf:
                new_trk = STrack(det_like, feat, max_obs=self.max_obs)
                ...
                if self.with_reid and feat is not None:
                    self._bank_add_feature(new_trk, feat)
        ...
        if lost_stracks:
            ...
            if _lost_ids:
                self._purge_from_coex(_lost_ids)
